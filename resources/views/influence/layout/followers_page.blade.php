      ...
      <hr>
        ...
        @foreach ($follow as $follow)
            <div class="row p-lg-5 ml-lg-5 ">
                <div class="col-lg-2">
                    <img src="{{asset('public/userimage/'.$follow->influencer_followers->user_image)}}" class="avatar-lg rounded-circle mt-3" alt="Shreyu">
                </div>
                <div class="col-lg-8 ml-lg-5">
                    <h5>{{ $follow->influencer_followers->full_name }}</h5>
                    <img src="{{asset('public\assets\ZURVOS_ASSETS\ICONS\Locations.png')}}" class="img-fluid" alt="Shreyu">
                    <span>{{ $follow->influencer_followers->location }}</span><br>
                    <form action="{{ url('influence/followw/'.$follow->id) }}" method="post">
                     @csrf
